...
class ImmunityRadiusConfig(ApiAppConfig):
    name = 'immunity_radius'
    label = 'immunity_radius'
    ...
    def connect_signals(self):
        ...
        User = get_user_model()
        from immunity_radius.api.freeradius_views import AccountingView
        ...
        pre_save.connect(
            organization_pre_save,
            sender=Organization,
            dispatch_uid='immunity_radius_org_pre_save',
        )
        post_save.connect(
            organization_post_save,
            sender=Organization,
            dispatch_uid='immunity_radius_org_post_save',
        )
        post_delete.connect(
            self.radiustoken_post_delete,
            sender=RadiusToken,
            dispatch_uid='immunity_radius_radiustoken_post_delete',
        )
        post_save.connect(
            self.radiusorgsettings_post_save,
            sender=OrganizationRadiusSettings,
            dispatch_uid='immunity_radius_organizationradiussettings_post_save',
        )
        post_delete.connect(
            self.radiusorgsettings_post_delete,
            sender=OrganizationRadiusSettings,
            dispatch_uid='immunity_radius_organizationradiussettings_post_delete',
        )
        post_save.connect(
            close_previous_radius_accounting_sessions,
            sender=RadiusAccounting,
            dispatch_uid='immunity_radius_close_previous_radius_accounting_sessions',
        )
        pre_save.connect(
            radius_user_group_change,
        ...
        if app_settings.CONVERT_CALLED_STATION_ON_CREATE:
            post_save.connect(
                convert_radius_called_station_id,
                sender=RadiusAccounting,
                dispatch_uid='immunity_radius_convert_called_station_id',
            )
