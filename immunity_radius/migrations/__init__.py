...
def delete_old_radius_token(apps, schema_editor):
    RadiusToken = get_swapped_model(apps, 'immunity_radius', 'RadiusToken')
...
def add_default_organization(apps, schema_editor):
    ...
    for model in models:
        Model = get_swapped_model(apps, 'immunity_radius', model)
    ...
    OrganizationRadiusSettings = get_swapped_model(
        apps, 'immunity_radius', 'organizationradiussettings'
    )
...
def add_default_groups(apps, schema_editor):
    Organization = get_swapped_model(apps, 'immunity_users', 'Organization')
    RadiusGroup = get_swapped_model(apps, 'immunity_radius', 'RadiusGroup')
...
def add_default_group_to_existing_users(apps, schema_editor):
    Organization = get_swapped_model(apps, 'immunity_users', 'Organization')
    OrganizationUser = get_swapped_model(apps, 'immunity_users', 'OrganizationUser')
    RadiusUserGroup = get_swapped_model(apps, 'immunity_radius', 'RadiusUserGroup')
    RadiusGroup = get_swapped_model(apps, 'immunity_radius', 'RadiusGroup')
    for organization in Organization.objects.all():
...
def populate_phonetoken_phone_number(apps, schema_editor):
    PhoneToken = get_swapped_model(apps, 'immunity_radius', 'PhoneToken')
