...
BATCH_PDF_TEMPLATE = get_settings_value(
    'BATCH_PDF_TEMPLATE',
    os.path.join(
        os.path.dirname(__file__), 'templates/immunity-radius/prefix_pdf.html'
    ),
)
...
# default is high because immunity-wifi-login-pages results always as 1 IP
...
try:
    PRIVATE_STORAGE_INSTANCE = import_string(
        get_settings_value(
            ...
            'immunity_radius.private_storage.storage.private_file_system_storage',
        )
    )
except ImportError:
...
COUNTERS_POSTGRESQL = (
    ...
    'immunity_radius.counters.postgresql.monthly_traffic_counter.MonthlyTrafficCounter',
)
COUNTERS_MYSQL = (
    'immunity_radius.counters.mysql.daily_counter.DailyCounter',
    'immunity_radius.counters.mysql.daily_traffic_counter.DailyTrafficCounter',
    'immunity_radius.counters.mysql.monthly_traffic_counter.MonthlyTrafficCounter',
)
COUNTERS_SQLITE = (
    'immunity_radius.counters.sqlite.daily_counter.DailyCounter',
    'immunity_radius.counters.sqlite.daily_traffic_counter.DailyTrafficCounter',
    'immunity_radius.counters.sqlite.monthly_traffic_counter.MonthlyTrafficCounter',
)
DEFAULT_COUNTERS = {
    ...
    'immunity_utils.db.backends.spatialite': COUNTERS_SQLITE,
}
