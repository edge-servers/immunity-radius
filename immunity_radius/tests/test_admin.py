...
class TestAdmin(
    ...
    app_label = 'immunity_radius'
    app_label_users = 'immunity_users'
    ...
    def test_radius_batch_csv_help_text(self):
        ...
        response = self.client.get(add_url)
        docs_link = (
            'https://immunity-radius.readthedocs.io/en/latest'
    ...
    def test_organization_radsettings_freeradius_allowed_hosts(self):
        ...
        with self.subTest('Empty IP list without FREERADIUS_ALLOWED_HOSTS'):
            ...
            with mock.patch('immunity_radius.settings.FREERADIUS_ALLOWED_HOSTS', []):
                response = self.client.post(url, form_data)
            self.assertContains(
                response,
                _(
...
_RADIUS_FREERADIUS_ALLOWED_HOSTS` is not provided.'
    ...
    def test_admin_menu_groups(self):
        # Test menu group (immunity-utils menu group) for RadiusAccounting, RadiusBatch,
