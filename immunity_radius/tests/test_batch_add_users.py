...
class TestCSVUpload(FileMixin, BaseTestCase):
    def test_generate_username_from_email(self):
        reader = [['', 'cleartext$password', 'rohith@immunity.com', 'Rohith', 'ASRK']]
        ...
        self.assertEqual(user.email, 'rohith@immunity.com')
    ...
    def test_generate_username_when_repeat(self):
        ...
        reader = [
            ['rohith', cleartext_password, 'rohith@immunity.com', 'Rohith', 'ASRK'],
            ['rohith', hashed_password, 'rohith@immunity.org', '', ''],
        ]
        batch = self._create_radius_batch(
            name='test', strategy='csv', csvfile=self._get_csvfile(reader)
        )
    ...
    def test_generate_password(self):
        reader = [['rohith', '', 'rohith@immunity.com', '', '']]
        batch = self._create_radius_batch(
            name='test', strategy='csv', csvfile=self._get_csvfile(reader)
        )
        batch.add(reader)
    ...
    def test_hashed_password(self):
        ...
        reader = [['rohith', hashed_password, 'rohith@immunity.com', 'Rohith', 'ASRK']]
        batch = self._create_radius_batch(
            name='test', strategy='csv', csvfile=self._get_csvfile(reader)
        )
        batch.add(reader)
...
class TestPrefixUpload(FileMixin, BaseTestCase):
    ...
    @patch('immunity_radius.settings.API_AUTHORIZE_REJECT', True)
