...
_BASE_COUNTER_CHECK = 'immunity_radius.counters.base.BaseCounter.check'
...
class TestFreeradiusApi(AcctMixin, ApiTokenMixin, BaseTestCase):
    ...
    @mock.patch('immunity_users.settings.USER_PASSWORD_EXPIRATION', 30)
    ...
    @mock.patch('immunity_radius.receivers.send_login_email.delay')
    ...
    @mock.patch('immunity_radius.receivers.send_login_email.delay')
    @mock.patch(
        'immunity_radius.api.serializers.RadiusAccountingSerializer.create',
    ...
    @mock.patch(
        'immunity_radius.receivers.send_login_email.delay', side_effect=OperationalError
    )
    @mock.patch('immunity_radius.receivers.logger')
    ...
    @mock.patch('logging.Logger.warn')
    @capture_any_output()
    def test_accounting_interim_update_immunity_closed_session(self, mocked_logger):
        org2 = self._create_org(name='org2', slug='org2')
...
class TestAutoGroupname(ApiTokenMixin, BaseTestCase):
    ...
    @mock.patch('immunity_radius.api.serializers.logging')
...
class TestClientIpApi(ApiTokenMixin, BaseTestCase):
    def setUp(self):
        ...
        self.freeradius_hosts_path = 'immunity_radius.settings.FREERADIUS_ALLOWED_HOSTS'
        self.client.post(
    ...
    def test_ip_from_setting_invalid(self):
        ...
        with mock.patch(
            'immunity_radius.settings.FREERADIUS_ALLOWED_HOSTS', ['localhost']
        ), mock.patch.object(
