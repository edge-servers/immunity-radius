...
from immunity_radius.api.serializers import (
    RadiusUserSerializer,
    UserGroupCheckSerializer,
)
from immunity_utils.tests import capture_any_output, capture_stderr
...
class TestApi(AcctMixin, ApiTokenMixin, BaseTestCase):
    ...
    @capture_any_output()
    @mock.patch('immunity_users.settings.AUTH_BACKEND_AUTO_PREFIXES', ['+39'])
    ...
    def test_organization_registration_enabled(self):
        ...
        with self.subTest('Test registration endpoint enabled by default'):
            r = self.client.post(
                url,
                {
                    'username': 'test@immunity.org',
                    'email': 'test@immunity.org',
                    'password1': 'password',
                    'password2': 'password',
                },
            )
        ...
        with self.subTest('Test registration endpoint disabled for org'):
            ...
            r = self.client.post(
                url,
                {
                    'username': 'test2@immunity.org',
                    'email': 'test2@immunity.org',
                    'password1': 'password',
                    'password2': 'password',
                },
            )
        ...
        with self.subTest('Test registration endpoint user global setting'):
            ...
            r = self.client.post(
                url,
                {
                    'username': 'test3@immunity.org',
                    'email': 'test3@immunity.org',
                    'password1': 'password',
                    'password2': 'password',
                },
            )
