...
from immunity_radius import tasks
from immunity_utils.tests import capture_any_output, capture_stdout
...
class TestTasks(FileMixin, BaseTestCase):
    ...
    def _get_expired_user_from_radius_batch(self):
        reader = [['sample', 'admin', 'user@immunity.com', 'SampleName', 'Lastname']]
    ...
    @mock.patch('immunity_radius.tasks.logger')
    @mock.patch('immunity_radius.utils.logger')
    ...
    def test_send_login_email(self, translation_activate, utils_logger, task_logger):
        ...
        radius_settings = organization.radius_settings
        radius_settings.login_url = 'https://wifi.immunity.org/default/login/'
        ...
        with self.subTest('do not fail if radius_settings missing'):
            ...
            utils_logger.warning.assert_called_with(
                ...
                'have any Immunity RADIUS settings configured'
            )
            translation_activate.assert_not_called()
