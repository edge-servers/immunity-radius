...
from immunity_radius.saml.utils import get_url_or_path
...
RadiusToken = swapper.load_model('immunity_radius', 'RadiusToken')
RegisteredUser = swapper.load_model('immunity_radius', 'RegisteredUser')
OrganizationRadiusSettings = swapper.load_model(
    'immunity_radius', 'OrganizationRadiusSettings'
)
User = get_user_model()
...
class TestAssertionConsumerServiceView(TestSamlMixin, TestCase):
    ...
    def test_user_registered_with_non_saml_method(self):
        ...
        with self.subTest('Test username remains unchanged'):
            with patch(
                'immunity_radius.settings.SAML_UPDATES_PRE_EXISTING_USERNAME', False
            ):
        ...
        with self.subTest('Test username gets updated'):
            with patch(
                'immunity_radius.settings.SAML_UPDATES_PRE_EXISTING_USERNAME', True
            ):
                saml_response, relay_state = self._get_saml_response_for_acs_view(
                    relay_state
                )
...
class TestLoginView(TestSamlMixin, TestCase):
    ...
    def test_saml_login_disabled(self):
        ...
        with self.subTest('SAML authentication is disabled site-wide'):
            with patch(
                'immunity_radius.settings.SAML_REGISTRATION_ENABLED', False
            ), patch.object(
