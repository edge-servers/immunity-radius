...
from immunity_radius.utils import get_organization_radius_settings
from immunity_utils.tests import capture_stderr

from .mixins import ApiTokenMixin, BaseTestCase

RadiusToken = load_model('immunity_radius', 'RadiusToken')
...
@patch('immunity_radius.settings.SOCIAL_REGISTRATION_ENABLED', True)
@patch.object(
...
class TestSocial(ApiTokenMixin, BaseTestCase):
    ...
    def test_redirect_cp_403(self):
        url = self.get_url()
        r = self.client.get(url, {'cp': 'http://wifi.immunity.org/cp'})
    ...
    def test_social_login_disabled(self):
        ...
        with self.subTest('Test social login disabled site-wide'):
            with patch(
                'immunity_radius.settings.SOCIAL_REGISTRATION_ENABLED', False
            ), patch.object(
                ...
                response = self.client.get(url, {'cp': 'http://wifi.immunity.org/cp'})
